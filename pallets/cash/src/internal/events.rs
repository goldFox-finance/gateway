...
use crate::{
    chains::{
        ...
        ChainId, ChainReorg, ChainReorgTally, Ethereum,
    },
    core::{apply_chain_event_internal, passes_validation_threshold, recover_validator},
    ...
    for block in blocks {
        if block.number() >= last.number() + 1 {
            ...
            if let Some(prior) = pending.get(offset) {
                ...
                pending[offset] = prior.clone().with_signer(validator); // XXX
            } else if offset == 0 {
...
pub fn receive_chain_reorg<T: Config>(
    ...
    let tally = if let Some(prior) = pending.iter().find(|&r| r.reorg == reorg) {
        ...
        prior.clone().with_signer(validator) // XXX
    } else {
